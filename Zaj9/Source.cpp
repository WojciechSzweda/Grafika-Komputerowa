...
typedef struct bpatch { // struktura dla wielomiana fragmentu Beziera 3 stopnia
	...
	POINT_3D anchors[4][7]; // siatka 4x4 punktów kontrolnych (anchor)
...
GLuint genBezier(BEZIER_PATCH patch, int divs) {
	...
	POINT_3D *last = new POINT_3D[divs + 1];
...
void GeneratePoints() {
	double startPosX = -1.0;
	double startPosY = -1.0;
	double startPosZ = -1.0;
	double addX = 0.5;
	for (size_t i = 0; i < 4; i++)
	{
		for (size_t j = 0; j < 7; j++)
		{
			mybezier.anchors[i][j] = POINT_3D{ startPosX + j * 0.5, startPosY + i * 0.5, startPosZ + addX };
			addX *= -1;
		}
		startPosZ += 0.5;
	}
...

{
	...
	GLboolean error = load_targa("ven0aaa2.tga", width_texture, height_texture, format, type, Ptr_mybezier_texture);
...
void Timer(int ms) {
	...
	for (size_t i = 0; i < 4; i++)
	{
		for (size_t j = 0; j < 7; j++)
		{
			mybezier.anchors[i][j].x = newBezier.anchors[i][j].x*sin(7 * 1.e-3*(t++));
			mybezier.anchors[i][j].y = newBezier.anchors[i][j].y*sin(7 * 1.e-3*(t++));
			mybezier.anchors[i][j].z = newBezier.anchors[i][j].z*sin(7 * 1.e-3*(t++));
