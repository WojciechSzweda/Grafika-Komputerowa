...
enum
{
	...
	TEXTURE_GIRL,                    // tekstura blonde_girl_white_skin_2
	TEXTURE_GIRL_UNC,                // teksura blonde_girl_white_skin_2 nieskompresowana
	TEXTURE_GIRL_GRAY,               // tekstura blonde_girl_white_skin_2_gray
	TEXTURE_GIRL_GRAY_UNC,           // tekstura blonde_girl_white_skin_2_gray nieskompresowana
...
GLuint GIRL, GIRL_UNC, GIRL_GRAY, GIRL_GRAY_UNC;
...
void GenerateTextures()
{
	...
	GLboolean error = load_targa("blonde_girl_white_skin_2.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku blonde_girl_white_skin_2.tga");
		//exit(0);
	}

	// utworzenie identyfikatora tekstury
	glGenTextures(1, &GIRL);

	// dowiązanie stanu tekstury
	glBindTexture(GL_TEXTURE_2D, GIRL);

	// definiowanie tekstury z kompresją
	glTexImage2D(GL_TEXTURE_2D, 0, GL_COMPRESSED_RGB, width, height, 0, format, type, pixels);

	// utworzenie identyfikatora tekstury
	glGenTextures(1, &GIRL_UNC);

	// dowiązanie stanu tekstury
	glBindTexture(GL_TEXTURE_2D, GIRL_UNC);
	...
	error = load_targa("blonde_girl_white_skin_2_gray.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku blonde_girl_white_skin_2_gray.tga");
		//exit(0);
	}

	// utworzenie identyfikatora tekstury
	glGenTextures(1, &GIRL_GRAY);

	// dowiązanie stanu tekstury
	glBindTexture(GL_TEXTURE_2D, GIRL_GRAY);

	// definiowanie tekstury z kompresją
	glTexImage2D(GL_TEXTURE_2D, 0, GL_COMPRESSED_LUMINANCE, width, height, 0, format, type, pixels);

	// utworzenie identyfikatora tekstury
	glGenTextures(1, &GIRL_GRAY_UNC);

	// dowiązanie stanu tekstury
	glBindTexture(GL_TEXTURE_2D, GIRL_GRAY_UNC);
...
void Menu(int value)
{
	...
	case TEXTURE_GIRL_GRAY_UNC:
	{
		texture = GIRL_GRAY_UNC;
		DisplayScene();
	}
	break;
...
int main(int argc, char *argv[])
{
	...
	glutAddMenuEntry("blonde_girl_white_skin_2.tga (kompresja)", TEXTURE_GIRL);
	glutAddMenuEntry("blonde_girl_white_skin_2.tga (bez kompresji)", TEXTURE_GIRL_UNC);
	glutAddMenuEntry("blonde_girl_white_skin_2_gray.tga (kompresja)", TEXTURE_GIRL_GRAY);
	glutAddMenuEntry("blonde_girl_white_skin_2_gray.tga (bez kompresji)", TEXTURE_GIRL_GRAY_UNC);
